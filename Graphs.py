...
def show_player_fg_pct_calendar(player_name=None):
    ...
    cursor.execute("""
        SELECT game_date, fg_pct, fgm, fga, team, min as minutes
        FROM player_stats 
        WHERE player = %s 
        ORDER BY game_date
    """, (player_name,))
    ...
    plt.tight_layout(rect=[0, 0.05, 0.9, 0.95])  # Make room for title and colorbar
    
    # Enable interactive mode and zooming
    plt.subplots_adjust(wspace=0.2, hspace=0.4)  # Add spacing between subplots
...
def show_player_points_calendar(player_name=None):
    ...
    # Calculate player's season average points
    ...
    plt.tight_layout(rect=[0, 0.07, 0.9, 0.95])  # Make room for title and colorbar
...
def analyze_fg_efficiency(player_name=None, top_n=5):
    ...
    cursor.execute("""
        SELECT game_date, team, fgm, fga, fg_pct
        FROM player_stats
        WHERE player = %s
        ORDER BY game_date
    """, (player_name,))
    ...
    total_fgm = sum(game[2] for game in games)
    total_fga = sum(game[3] for game in games)
    avg_fg_pct = total_fgm / total_fga if total_fga > 0 else 0
    
    # Calculate hot/cold streaks
    fg_pcts = [game[4] for game in games if game[3] > 0]  # Only games with attempts
    best_fg = max(fg_pcts) if fg_pcts else 0
    worst_fg = min(fg_pcts) if fg_pcts else 0
    
    # Find best/worst games
    best_game_idx = next((i for i, game in enumerate(games) if game[3] > 0 and game[4] == best_fg), None)
    worst_game_idx = next((i for i, game in enumerate(games) if game[3] > 0 and game[4] == worst_fg), None)
    ...
    if best_game_idx is not None:
        best_game = games[best_game_idx]
        print(f"\nBest Shooting Game: {best_game[0]} vs {best_game[1]}")
        print(f"  {best_game[2]}-{best_game[3]} ({best_fg:.1f}%)")
    
    if worst_game_idx is not None:
        worst_game = games[worst_game_idx]
    ...
    for game in games:
        ...
        print(f"{date:<12} {team:<10} {fgm}-{fga:<10} {fg_pct:.1f}%")
...
def show_player_fg_attempts_calendar(player_name=None):
    ...
    for i in range(num_months):
        ...
        # Find the max deviation from average to set color scale symmetrically
        max_dev = np.nanmax(np.abs(cal_data_fgpct))
        max_dev = max(max_dev, 15)  # At least 15 percentage points for visibility
    ...
    # Create tick formatter that shows percentages with one decimal place
    import matplotlib.ticker as mtick
    cbar.ax.yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1, symbol='±'))
